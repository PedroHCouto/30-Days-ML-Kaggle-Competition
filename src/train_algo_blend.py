...
def run(k_fold, test_pred = False):
    ...
    model_dic = {'XGB_1': XGBRegressor,
                 ...
                 'XGB_4': XGBRegressor,
                 'XGB_5': XGBRegressor,}
    model_save_name = f"{'_'.join([key for key in model_dic.keys()])}_w_hyp_BLEND"
    ...
    for i in tqdm(range(k_fold)):
        ...
        valid_blend_error.append(rmse_blend_valid)

        if test_pred:
            pred_blend_test = model.predict(test_result_df)
            test_blend_pred.append(pred_blend_test)

    ### saving submission file in case predictions were done
    if test_pred:
        test_output = pd.DataFrame(
            {'id': df_test.id,
            'target': np.mean(np.column_stack(test_blend_pred), axis=1)
        })        
        time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        output_path = os.path.join(config.PREDICTIONS_PATH, f'{time}_{model_save_name}.csv')
    ...
    print(f'Bled valid rmse for the {k_fold} folds are: {valid_blend_error}')
    print(f'RMSE average for the blend:: {np.mean(valid_blend_error)}')
    print(f'RMSE std for the blend:: {np.std(valid_blend_error)}')
